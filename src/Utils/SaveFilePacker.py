...
class SaveFilePacker(object):
    ...
    def pack_databases(_dir):
        ...
        mmaps = [
            SaveFilePacker.get_db_mmap(os.path.join(_dir, MAIN_DB_NAME)),
            SaveFilePacker.get_db_mmap(os.path.join(_dir, BACKUP_DB_NAME)),
            SaveFilePacker.get_db_mmap(os.path.join(_dir, BACKUP_DB2_NAME))
        ]
    ...
    def do_pack(from_folder, to_file):
        ...
        for _bytes in SaveFilePacker.pack_databases(from_folder):
    ...
    def do_unpack(from_file, to_folder):
        ...
        for dump_name, db_size in databases.items():
            ...
            if db_size == 0:
                break
            SaveFilePacker.create_db(dump_name, decompressed_dbs, _start, _start+db_size)
            _start += db_size
    ...
    def process_unpack(input_file, result_dir):
        ...
        SaveFilePacker.do_unpack(input_file, result_dir)


    def process_repack(input_dir, result_file):
        SaveFilePacker.do_pack(input_dir, result_file)
